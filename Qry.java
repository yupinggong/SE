...
//  private int docIteratorScoreCache = Qry.INVALID_DOCID;
//  //Yuping sep 14
...
//  /**
//   *  Return the score of the document that the iterator points to now.
//   *  Use docIteratorHasMatch to determine whether the iterator
//   *  currently points to a document.  If the iterator doesn't point
//   *  to a document, an invalid document id is returned.
//   *  @return The internal id of the current document.
//   */
//  //Yuping sep14
//  public int docIteratorGetMatchScore () {
//    if (this.docIteratorHasMatchCache ()) {
//      return this.docIteratorScoreCache;
//    } else {
//      throw new IllegalStateException("No matching docid was cached.");
//    }
//  }
  ...
  protected boolean docIteratorHasMatchAll (RetrievalModel r) {
    ...
    while (! matchFound) {
      ...
      //int docScore = q_0.docIteratorGetMatchScore();
      ...
      for (int i=1; i<this.args.size(); i++) {
		...
		if (docid_0 != docid_i) {	// docid_0 can't match.  Try again.
		  ...
		  //docScore = Math.min(docScore, q_i.docIteratorGetMatchScore());	
		}
      }

      if (matchFound) {
        docIteratorSetMatchCache (docid_0);
        //docIteratorSetScoreCache (docScore);
...
//      int docScore = ((QryIop)q_0).docIteratorGetMatchPosting().tf;
//      //Yuping sep 14
      //int tf = ((QryIop) q_0).docIteratorGetMatchPosting().tf;
      // Yuping sep 13
      this.docIteratorSetMatchCache (docid);
//      this.docIteratorSetScoreCache (docScore);
//      //Yuping sep 14
      return true;
    } else {
      return false;
  ...
  protected boolean docIteratorHasMatchMin (RetrievalModel r) {
    ...
    //int maxScore = Qry.INVALID_DOCID;
    for (int i=0; i<this.args.size(); i++) {
      Qry q_i = this.args.get(i);
      if (q_i.docIteratorHasMatch (r)) {
        int q_iDocid = q_i.docIteratorGetMatch ();
//        int q_iDocScore = q_i.docIteratorGetMatchScore();
//        //System.out.println(q_iDocScore);
//        //Yuping sep 14
        if ((minDocid > q_iDocid) ||
            (minDocid == Qry.INVALID_DOCID)) {
          minDocid = q_iDocid;
          //maxScore = q_iDocScore;
        }
//        } else if (minDocid == q_iDocid) {
//          maxScore = Math.max(maxScore, q_iDocScore);
//          }
      }
    }
    ...
    if (minDocid != Qry.INVALID_DOCID) {
      ...
      //docIteratorSetScoreCache (maxScore);
      return true;
    } else {
      return false;
    }
   ...
   */
//  //Yuping sep 14
//  private void docIteratorSetScoreCache (int docScore) {
//    this.docIteratorScoreCache = docScore;
//  }
