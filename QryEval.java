...
//    if(parameters.get("retrievalAlgorithm").equals("UnrankedBoolean")){
//    	model = new RetrievalModelUnrankedBoolean();
//    }
//    else if(parameters.get("retrievalAlgorithm").equals("RankedBoolean")){
//		
//	}
//    else {
//    	
//    }
	...
	try{
		output = new BufferedWriter(new FileWriter(new File(parameters.get("trecEvalOutputPath"))));
	}
	catch(IOException e){
		System.err.println("Error: Result file can't be created");
		System.exit(1);
	}
	//  Configure output fiel
  ...
  static Qry parseQuery(String qString, RetrievalModel model) throws IOException {
	...
	// Not done yet.  Add the current operator as an argument to
        ...
        currentOp.setDisplayName (token);
        opStack.push(currentOp);
      } else if (token.length() > 6 && token.substring(0, 6).equalsIgnoreCase("#near/") && token.substring(6).matches("^[1-9][0-9]*$")) {
    ...
    if (q != null) {
      ...
      if (q.args.size () > 0) {		// Ignore empty queries
        ...
        while (q.docIteratorHasMatch (model)) {
          ...
          //double score = q.docIteratorGetMatchScore();
          double score = ((QrySop) q).getScore (model); // getScore according to model
          r.add (docid, score);
  ...
  static void processQueryFile(String queryFilePath,
                               RetrievalModel model,BufferedWriter output)
      throws IOException {
  ...
  static void printResults(String queryName, ScoreList result,BufferedWriter output) throws IOException {
    ...
    if (result.size() < 1) {
    	output.write("10 Q0 dummy 1 0 yuping");
    } else {
    	
      result.sort();;	
      for (int i = 0; i < Math.min(100, result.size()); i++) {
        output.write(queryName+"\t" +"Q0\t"+ Idx.getExternalDocid(result.getDocid(i)) + "\t" + (i+1) + "\t" +
            + result.getDocidScore(i) + "\t" + "yuping"+"\n");
      }
    }
  }
